...
Following the standard preprocessing pipeline, the CONN toolbox [@whitfield-gabrieli2012] takes care of the FC generation after denoising [@behzadi2007]. This is an add-on to SPM that can be downloaded [here](https://www.nitrc.org/projects/conn/).
...
3.  Click done to generate the .mat file. Under the path /conn_example/results/firstlevel/RRC, you should now visualize the .mat files for each subject.
    ...
    ![In Matlab: connectome = load('resultsROI_Subject001_Condition001.mat'); W = connectome.Z; figure, imagesc(W, \[0 1\]), xlabel(\[' 131 LANG regions '\]), ylabel('131 LANG regions'), title(\[' Functional connectome '\]); colormap(jet), colorbar, set(gcf,'color','w'), set(gca,'fontsize',14)](/images/FC.png){fig-align="center"}

**Learn how to analyze these functional connectomes with graph theory [here](../GT/graphvar.qmd).**
